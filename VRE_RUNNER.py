...
from agent.VRE_Agent import dpfrepAgent
...
class Wrapper:
    ...
    def __init__(self, configuration=None):
        ...
        :param configuration: A dictionary containing parameters that define how the operation should be carried out,
            which are specific to DpFrEP agent.
    ...
    def run(self, input_files, input_metadata, output_files, output_metadata):
        """
        Main run function for running DpFrEP agent.
...
def main_wrapper(config_path, in_metadata_path, out_metadata_path):
    ...
    :param out_metadata_path: Path to write the VRE JSON file containing information on agent outputs.
    ...
    try:
        ...
        logger.progress("DpFrEP agent successfully executed; see {}".format(out_metadata_path))
        return result

    except Exception as error:
        errstr = "DpFrEP agent wasn't successfully executed. ERROR: {}.".format(error)
        logger.error(errstr)
        raise Exception(errstr)


if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="VRE DpFrEP Agent")
